...
""" Created on Oct 2024    @author: user
    Operadores Evolutivos Guiados por Feromonas (ACO-Enhanced) para DeepGA V10:
    - Matriz de Feromonas Arquitectónicas (Pheromone Trail Matrix para NAS).
...
class PheromoneMatrix:
    """
    Matriz de Feromonas Arquitectónicas para Neural Architecture Search (ACO-NAS).
    Mantiene y actualiza los rastros de feromona en cada decisión arquitectónica de DeepGA:
    - Número de capas convolucionales y densas.
    - Filtros, tamaño de kernel, tipo y tamaño de pooling por posición de capa.
    - Neuronas en capas densas por posición.
    - Conexiones residuales (skip-connections) entre capas.
    """
    def __init__(self, min_conv: int = 2, max_conv: int = 5,
        ...
        self.alpha = alpha      # Sensibilidad / Explotación de feromonas
        ...
        # Identificador de conexión: "src_to_dst" (ej. "0->2", "1->3")
        self.skip_tau = {}
    ...
    def deposit(self, population: list, top_k_ratio: float = 0.35, delta_factor: float = 1.5):
        """
        Deposita feromonas sobre las decisiones arquitectónicas de los individuos élite.
        Δτ = delta_factor * ((fitness - f_min) / (f_max - f_min + eps))
        """
        if not population:
            return

        sorted_pop = sorted(population, key=lambda ind: ind[1], reverse=True)
        k = max(1, int(len(sorted_pop) * top_k_ratio))
        elite = sorted_pop[:k]

        f_vals = [ind[1] for ind in population]
        f_max = max(f_vals)
        f_min = min(f_vals)
        f_span = max(1e-6, f_max - f_min)

        for ind in elite:
            genome = ind[0]
            fitness = ind[1]
            # Intensidad de depósito proporcional a la calidad del individuo
            delta_tau = delta_factor * ((fitness - f_min) / f_span)
...
def guided_adaptive_mutation_v10(x, pheromones: PheromoneMatrix, mr_rate: float,
                                 parent_fitness: float = 0.5, f_avg: float = 0.5,
                                 min_conv: int = 2, max_conv: int = 5,
                                 min_full: int = 1, max_full: int = 4):
    """
    Operador de Mutación Adaptativa Guiada por Feromonas (V10):
    1. Decide si mutar con la tasa adaptativa individual `mr_rate`.
    2. Modula la intensidad de mutación (Micro vs Macro).
    3. Cada nuevo hiperparámetro o conexión se extrae de la distribución de feromonas en vez de azar ciego.
    """
    ...
    if is_high_performer:
        # Micro-ajustes guiados para individuos buenos
        mut_choice = random.choices(['modify_layer', 'flip_skip', 'add_remove_layer'], weights=[0.55, 0.25, 0.20])[0]
    else:
        # Re-estructuración guiada para individuos que necesitan explorar
        ...
        if n_c > 2:
            ...
            if j < len(adj_matrix) and i < len(adj_matrix[j]):
                ...
                if conn_key in pheromones.skip_tau:
                    # Muestra el estado del skip guiado por feromonas
