...
def _evaluate_individual(e, n_channels: int, out_size: int, n_classes: int, device: torch.device,
    ...
    """Decodifica y entrena concurrentemente una única CNN."""
...
def green_DeepGA_v3(execution: int, memoryC: bool, train_epochs: int, train_dl: DataLoader, val_dl: DataLoader, lr: float,
                    ...
                    num_workers: int = 2, k_neighbors: int = 3, prune_quantile: float = 0.25,
                    max_prune_retries: int = 3, dataset_name: str = "CIFAR-10", seed: int = None, **kwargs):
    """
    Algoritmo DeepGA V3: Podado por Familiaridad y Linaje + Entrenamiento Concurrente.
    ...
    if target_load:
        print("Re-Initialize population (DeepGA V3)")
        ...
        print('Initialize population (DeepGA V3 - Kinship Pruning + Parallelism)')
        ...
        while len(pop) < N:
            ...
            for (e, strID), (fit, acc, pars) in zip(batch_genomes, results):
                ...
                evaluated_history.append((e, fit, acc))
                evals += 1
    ...
    print('--------------------------------------------')
    while t < T:
        print(f'Generation: {t} (DeepGA V3)')
        ...
        print(f" [V3 Pruning Threshold (peor {int(prune_quantile*100)}%)]: {prune_threshold:.4f}")
        ...
        while len(offspring) < int(N/2):
            ...
            # Evaluar en paralelo las CNNs supervivientes
            candidates = [c1, c2]
            ...
            if to_train:
                with ThreadPoolExecutor(max_workers=len(to_train)) as executor:
                    ...
                    for idx, strID, fut in futures:
                        ...
                        if memoryC and strID is not None:
                            cacheM[strID] = [fit, acc, pars]
                        evaluated_history.append((candidates[idx], fit, acc))
                        evals += 1
                        print(f"[Parallel Offspring V3] Fit: {fit:.4f}, Acc: {acc:.2f}%, Params: {pars}")
    ...
    try:
        ...
        print(f"Nota al guardar mejor modelo V3: {e}")
...
def final_evaluation(execution: int, bestind: list, train_dl: DataLoader, val_dl: DataLoader, lr: float,
                     ...
                     n_channels: int = 3, n_classes: int = 10, out_size: int = 32, variant: str = "v3", auto_download: bool = False):
